...
export function Sidebar({ className, items, defaultCollapsed = false, defaultOpen = false, ...props }: SidebarProps) {
            ...
            <div className="flex-1 overflow-auto py-2">
              <nav className="grid gap-1 px-2">
                {items.map((item, index) => (
                  <SidebarItem key={index} item={item} pathname={pathname} isCollapsed={false} />
                ))}
              </nav>
            </div>
          </div>
...
const itemVariants = cva(
  "flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground transition-all",
  {
    variants: {
      active: {
...
function SidebarItem({
  ...
  const [isExpanded, setIsExpanded] = React.useState(item.isExpanded)
      ...
      <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
        <CollapsibleTrigger asChild>
          <Button
            variant="ghost"
            className={cn(
              itemVariants({ active: isActive, isCollapsed }),
              "w-full justify-between",
              isCollapsed && "px-2",
            )}
          >
            <div className="flex items-center gap-2">
              {Icon && <Icon className={cn("h-5 w-5", isCollapsed && "h-5 w-5")} />}
          ...
          <CollapsibleContent className="pl-6 pt-1">
            <nav className="grid gap-1">
              {item.items.map((subItem, index) => (
                <SidebarSubItem key={index} item={subItem} pathname={pathname} />
              ))}
            </nav>
          </CollapsibleContent>
        )}
  ...
  return (
    <Link
      href={item.href || "#"}
      className={cn(itemVariants({ active: isActive, isCollapsed }), "w-full", isCollapsed && "px-2 justify-center")}
